...
class PDFUploadForm(forms.Form):
    ...
    max_imovel = forms.FloatField(initial=-1, required=False)
    max_res = forms.FloatField(initial=-1, required=False)
    
    # --- AQUI ESTÁ A CORREÇÃO ---
    # Mudamos o widget para HiddenInput.
    debug_regions = forms.BooleanField(
        initial=False, 
        required=False,
        widget=forms.HiddenInput() # Adicione este parâmetro
    )